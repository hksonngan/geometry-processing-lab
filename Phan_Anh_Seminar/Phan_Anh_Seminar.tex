...
\begin{equation}
	\begin{array}{lcl}
	\mathbf{w^T x_n} + b \geq +1\ for\ t_n = +1\\
	\mathbf{w^T x_n} + b \leq -1\ for\ t_n = -1
	\end{array}
\end{equation}

Combined in one equation, this can be written as:
\begin{equation}
t_n(\mathbf{w^T x_n} + b) \geq 1\ \forall n
\label{eq:margin}
\end{equation}
...
subject to the constraint in equation $\ref{eq:margin}$. This quadratic programming problem with linear constraints can be solved efficiently using Lagrange multipliers. More detail on how to solve the above optimization problem and extensions to the non-separable case as well as non-linear support vector machines can be found in \cite{burges1998tutorial}.
