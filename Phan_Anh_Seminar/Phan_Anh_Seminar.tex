...
The solution for $\mathbf{w}$ is nothing more than a linear combination of training examples:

\begin{equation}
\mathbf{w} = 
\end{equation}

Note that $a_i > 0$ only for training data points on the margin called support vectors.

More detail on extensions to the  case of non-separable data as well as non-linear support vector machines can be found in \cite{burges1998tutorial}.

%\subsubsection{Structured SVM learning framework}

%\cite{tsochantaridis2006large}

\subsubsection{SVM for region classification}
