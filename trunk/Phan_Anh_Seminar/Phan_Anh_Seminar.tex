...
For the image segmentation problem, our goal is to minimize the energy function associated with a MRF constructed from the training data and the observed input image. Particularly, we need to find a labelling configuration $A = (A_1, ..., A_{\lvert P \rvert})$ that minimizes an energy $E(A)$ given by
