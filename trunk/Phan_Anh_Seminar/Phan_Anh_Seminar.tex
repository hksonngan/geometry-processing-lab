...
In the field of image processing, we often use MRF to capture the correlation between hidden true states of pixels and the observed image data. The spatial coherent of the image content can also be modeled using the neighborhood relationships between pixels\textquotesingle \ state variables as shown in Figure $\ref{fig:mrf}$.
For the image segmentation problem, our goal is to minimize the energy function associated with a MRF constructed from the training data and the observed input image.
