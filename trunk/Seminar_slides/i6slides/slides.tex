...
\end{definition}
\begin{definition}
A random walk on graph $G = (V, E, W)$ is a Markov process with transition matrix $P = D^{-1}W$, where $D = \mbox{diag}(\sum_{u}W_{vu})$.
\end{definition}
\begin{itemize}
\item $\pi$ is a stationary distribution on graph $G$ if $\pi P = \pi \Leftrightarrow \pi_u = \sum\limits_{v, v \rightarrow u} \pi_v P_{vu}$
