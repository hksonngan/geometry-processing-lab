...
\label{eq:explicit}
\end{equation}
where $s \in [0, 1]$ is a damping factor controlling the magnitude of the update vector thereby stabilizing the flow. $M^{-1}$ is the inverse of the mass matrix $M \in \mathcal{R}^{m \times m}$ of the mesh $M^j_h$ at time step $j$. It is derived in the finite element formulation for the Dirichlet problem \cite{Wardetzky08Cotan} and has the following form:
\begin{equation}
M_{pq} = 
