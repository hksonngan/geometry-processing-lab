...
Figure $\ref{fig:volume}$ illustrates how the volume of a surface is calculated. Note that w.l.o.g. the model can always be translated to the origin since translation does not scale the volume and that in an occluded region (where the normal pointing to the origin) the volume of a tetrahedron can be negative hence only the volume enclosed by that region is added to the total volume.
...
Since $vol \ M_h$ linearly depends on $p$, the gradient of $vol \ M_h$ at $p$ is easily derived as follows:
